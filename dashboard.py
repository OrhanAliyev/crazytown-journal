...
import numpy as np
import time
import ccxt  # PROFESYONEL KRİPTO VERİ KÜTÜPHANESİ
...
TRANSLATIONS = {
    "EN": {
        ...
        "run_ai": "SCAN MARKET", "ai_analyzing": "Connecting to Binance Node...", 
...
def get_crypto_data(symbol, timeframe):
    """
    CCXT kullanarak Binance'den veri çeker.
    Rastgele simülasyonu kaldırıp GERÇEK veri odaklıdır.
    """
    try:
        exchange = ccxt.binance()
        # Sembol düzeltme (Kullanıcı 'BTC' girerse 'BTC/USDT' yap)
        if "/" not in symbol:
            symbol = f"{symbol.upper()}/USDT"
        else:
            symbol = symbol.upper()
            
        # Timeframe mapping (Streamlit -> CCXT)
...
# TAB 5: AI LAB (CCXT ILE GERÇEK VERİ)
with tab5:
    st.write("")
