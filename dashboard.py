...
TRANSLATIONS = {
    "EN": {
        ...
        "run_ai": "RUN ANALYSIS", "ai_analyzing": "Connecting to CoinGecko...", "ai_calc": "Running Technical Models...",
    ...
    "TR": {
        ...
        "ai_title": "🤖 QUANT YAPAY ZEKA MOTORU", "ai_desc": "CoinGecko verileri ve olasılıksal tahmin modelleri.",
        "run_ai": "ANALİZİ BAŞLAT", "ai_analyzing": "CoinGecko'ya Bağlanılıyor...", "ai_calc": "Hesaplanıyor...",
...
def get_market_data_robust(symbol, interval):
    ...
    1. CoinGecko (Primary)
    2. Binance (Secondary)
    3. Simulation (Safe Fallback)
    """
    # 1. COINGECKO
    try:
        cg_id = {"BTC/USDT": "bitcoin", "ETH/USDT": "ethereum", "SOL/USDT": "solana"}.get(symbol, "bitcoin")
    ...
    try:
        ...
        url_bin = f"https://api.binance.com/api/v3/klines?symbol={sym_bin}&interval={interval}&limit=100"
    ...
    return pd.DataFrame({'time': dates, 'close': prices}), "Simulation (Network Offline)"
...
# TAB 5: AI LAB (YENİLENMİŞ MOTOR - ASLA ÇÖKMEZ)
with tab5:
    st.write("")
